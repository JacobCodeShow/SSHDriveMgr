...
class MountResult:
    success: bool
    message: str
    drive_letter: str = None  # resolved actual drive letter (set when AUTO was used)
...
class SSHFSController:
    ...
    def mount(self, conn: Connection, disable_cache: bool = False,
        ...
        if result.success:
            result.drive_letter = conn.drive_letter
    ...
    def _mount_direct(self, conn: Connection, disable_cache: bool = False,
        ...
        # AUTO drive-letter: resolve to the smallest free letter right now
        from src.drive_utils import is_auto_drive, resolve_drive_letter
        if is_auto_drive(conn.drive_letter):
            resolved = resolve_drive_letter(conn.drive_letter)
            if resolved is None:
                return MountResult(False, "No free drive letter available.")
            conn.drive_letter = resolved
