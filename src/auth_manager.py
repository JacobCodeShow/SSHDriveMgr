# auth_manager.py – Benutzerverwaltung und Session für SSH 磁盘管理器.
#
# Konzept:
...
class AuthManager:
    ...
    # ------------------------------------------------------------------
    # Remember-password (machine-bound encrypted storage)
    # ------------------------------------------------------------------

    @staticmethod
    def _ensure_remembered_table() -> None:
        """Create the remembered_login table if it doesn't exist yet."""
        with get_connection() as conn:
            conn.execute("""
    ...
    def remember_credentials(username: str, password: str) -> bool:
        """Encrypt and store login credentials for automatic fill next time.

        The password is encrypted with a machine-bound key (see machine_crypto).
        Only one set of credentials is stored (id=1); calling this again
        overwrites the previous entry.
        """
        from src.machine_crypto import machine_encrypt
        if not username or not password:
            return False
        AuthManager._ensure_remembered_table()
        pw_enc = machine_encrypt(password)
        with get_connection() as conn:
            conn.execute(
                """INSERT INTO remembered_login (id, username, password_enc)
                   VALUES (1, ?, ?)
                   ON CONFLICT(id) DO UPDATE SET
                       username = excluded.username,
                       password_enc = excluded.password_enc,
                       created_at = datetime('now')""",
                (username.strip(), pw_enc)
            )
            conn.commit()
        return True

    @staticmethod
    def get_remembered_credentials() -> Optional[tuple[str, str]]:
        """Retrieve and decrypt stored credentials.

        Returns (username, password) or None if nothing is stored or
        decryption fails (e.g. running on a different machine).
        """
        from src.machine_crypto import machine_decrypt
        AuthManager._ensure_remembered_table()
        with get_connection() as conn:
            row = conn.execute(
                "SELECT username, password_enc FROM remembered_login WHERE id = 1"
            ).fetchone()
        if not row:
            return None
        password = machine_decrypt(row["password_enc"])
        if password is None:
            # Ciphertext can't be decrypted on this machine — clean up stale entry
    ...
    def clear_remembered_credentials() -> None:
        ...
        AuthManager._ensure_remembered_table()
        with get_connection() as conn:
            conn.execute("DELETE FROM remembered_login WHERE id = 1")
            conn.commit()
...
class UserConnectionManager:
    ...
    def duplicate(self, conn_id: str, new_name: str = None):
        """Duplicate a connection at the database level.

        Copies all columns verbatim (including encrypted password and metadata),
        assigns a new id, appends " (copy)" to the name, and puts it at the end
        of the sort order. Returns the new Connection, or None if not found.
        """
        with get_connection() as conn:
            row = conn.execute(
                "SELECT * FROM connections WHERE id = ? AND user_id = ?",
            ...
            # Decrypt original name, then re-encrypt with (copy) suffix
            orig_name = self._dec_meta(row, "name_enc", "name_iv", "name")
            copy_name = new_name if new_name else (orig_name + " (copy)")
            name_enc, name_iv = self._encrypt_pw(copy_name)
            data["name"] = ""  # plaintext column stays empty (post-migration convention)
            data["name_enc"] = name_enc
            data["name_iv"] = name_iv

            # Put the copy at the end of the sort order
            max_order = conn.execute(
                "SELECT COALESCE(MAX(sort_order), 0) FROM connections WHERE user_id = ?",
                (self._user.id,)
            ).fetchone()[0]
            ...
            conn.execute(
                """INSERT INTO app_settings
                   (user_id, start_with_windows, minimize_to_tray,
                    ...
                    auto_remount_on_lost, sshfs_disable_cache,
                    updated_at)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'))
