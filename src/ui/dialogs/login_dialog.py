"""
login_dialog.py – Login-Dialog for SSH 磁盘管理器.
...
class LoginWorker:
    """Run the blocking AuthManager.authenticate() in a Python thread.

    Why not QThread: under the modal exec() event loop used by main.py, QThread
    start() is delayed by more than a second on PyQt6 (verified by diagnostics),
    which made the dialog sit frozen after clicking "Sign in".  A plain Python
    thread starts immediately; the result is marshalled back to the GUI thread
    with QTimer.singleShot(0).
    """
    def __init__(self, username: str, password: str, dialog):
        self._username = username
        self._password = password
        self._dialog = dialog  # receiver on the GUI thread (cross-thread signal)
        self._thread = threading.Thread(
            target=self._run, daemon=True, name="login-worker"
        )

    def _run(self):
        try:
            user = AuthManager.authenticate(self._username, self._password)
            if user is None:
                status = ("fail", None)
            else:
                status = ("ok", user)
        except LoginLockedError as e:
            status = ("lockout", (e.seconds_remaining, e.total_failures))
        except Exception as e:
            status = ("error", str(e))
        # Cross-thread signal: queued to the dialog's thread, delivered by the
        # (modal) event loop. QTimer.singleShot(0) from a worker thread would
        # create a timer owned by that thread (no event loop) and never fire.
        self._dialog._auth_finished.emit(status[0], status[1])
...
class LoginDialog(FramelessDialog):
    ...
    _auth_finished = pyqtSignal(str, object)
    ...
    def __init__(self, parent=None):
        ...
        self._build_ui()
        self._auth_finished.connect(self._on_auth_done)
    ...
    def _build_login_form(self, layout: QVBoxLayout):
        ...
        # Remember-password checkbox (machine-bound encrypted storage)
        self._remember_cb = QCheckBox(tr("login.remember_password"))
        self._remember_cb.setObjectName("rememberPasswordCb")
        ...
        # Auto-fill remembered credentials (if any, decryptable on this machine)
        self._load_remembered_credentials()

        self._login_user.setFocus()

    def _load_remembered_credentials(self):
        """Try to load and decrypt stored credentials; fill the form if successful."""
        try:
            creds = AuthManager.get_remembered_credentials()
        except Exception:
            creds = None
        if creds:
            ...
            # Put focus on the login button so the user can just press Enter
    ...
    def _do_login(self):
        ...
        # Disable inputs and show "Signing in..." state.  The actual auth
        # runs in a QThread so the UI stays responsive — no freeze, no jitter.
        self._login_user.setEnabled(False)
        self._login_pw.setEnabled(False)
        self._remember_cb.setEnabled(False)
        self._login_btn.setEnabled(False)
        self._login_btn_original_text = self._login_btn.text()
        self._login_btn.setText(tr("login.signing_in"))
        self._login_error.setVisible(False)

        self._auth_worker = LoginWorker(username, password, self)
        self._auth_worker.start()

    def _restore_login_controls(self):
        """Re-enable login inputs and restore the button text after a failed attempt."""
        self._login_user.setEnabled(True)
        self._login_pw.setEnabled(True)
        self._remember_cb.setEnabled(True)
        self._login_btn.setEnabled(True)
        if hasattr(self, "_login_btn_original_text"):
            self._login_btn.setText(self._login_btn_original_text)
        self._update_login_btn_state()

    def _on_auth_done(self, status: str, payload):
        """Dispatch the worker result on the GUI thread."""
        if status == "ok":
            username = self._login_user.text().strip()
            password = self._login_pw.text()
            self._on_auth_ok(payload, username, password)
        elif status == "fail":
            self._on_auth_fail()
        elif status == "lockout":
            seconds, failures = payload
            self._on_auth_lockout(seconds, failures)
        else:
            self._on_auth_error(payload)

    def _on_auth_ok(self, user, username: str, password: str):
        """Auth succeeded — log in, persist credentials, close dialog."""
        Session.login(user)
        # Persist or clear remembered credentials (machine-bound encryption).
        try:
            if self._remember_cb.isChecked():
                AuthManager.remember_credentials(username, password)
            else:
    ...
    def _on_auth_fail(self):
        """Wrong username/password — re-enable controls and show error."""
        self._restore_login_controls()
        self._show_login_error(tr("login.invalid"))
        self._login_pw.clear()
        self._login_pw.setFocus()

    def _on_auth_lockout(self, seconds: int, failures: int):
        """Account locked — re-enable controls and start countdown."""
        self._restore_login_controls()
        self._start_lockout_countdown(seconds, failures)
        self._login_pw.clear()

    def _on_auth_error(self, msg: str):
        """Unexpected error during auth — re-enable controls and show error."""
        self._restore_login_controls()
