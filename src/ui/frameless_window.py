...
class FramelessMainWindow(QMainWindow):
    ...
    def __init__(self, parent=None) -> None:
        ...
        # Stay hidden through the entire build: setWindowFlag() below rebuilds
        # the native window, and Windows may briefly show it otherwise - that
        # leaves a thin edge (the 'small block') visible before showMaximized().
        self.hide()
        self.setWindowFlag(Qt.WindowType.FramelessWindowHint)
        self.hide()
        self.setMouseTracking(True)
        # Fill background with themed palette color on first paint to avoid white flash
    ...
    def setCentralWidget(self, widget: QWidget) -> None:  # noqa: N802
        ...
        raw_title     = self.windowTitle() or "SSH 磁盘管理器"
