...
class ConnectionCard(QFrame):
    ...
    drive_change_requested = pyqtSignal(str)  # click drive badge (unmounted) to pick drive
    ...
    def _badge_text(self) -> str:
        ...
        dl = (self._conn.drive_letter or "").upper()
        if dl in ("AUTO", ""):
            return "Auto"
    ...
    def update_mount_state(self, mounted: bool):
        ...
        if mounted:
            ...
            self._drive_badge.setToolTip("Click to choose drive letter (or Auto)")
            self._drive_badge.setCursor(Qt.CursorShape.PointingHandCursor)
    ...
    def _on_drive_badge_clicked(self, event):
        ...
        if self._is_ftp:
            ...
            # Unmounted: let the user quickly pick a drive letter (or Auto)
            self.drive_change_requested.emit(self._conn.id)
