"""
main_window.py – The primary application window for SSH 磁盘管理器.
"""
...
class MainWindow(FramelessMainWindow):
    ...
    def __init__(self):
        ...
        self._clipboard_conn_id: str | None = None  # for copy/paste (Ctrl+C/V)
        ...
        logger.info("SSH 磁盘管理器 gestartet.")
    ...
    def _setup_ipc(self):
        import threading
        self._ipc_pipe_name = r"\\.\pipe\SSHDriveMgr_IPC_v1"
    ...
    def _create_connection_container(self, conn, mounted):
        ...
        card.drive_change_requested.connect(self._on_drive_change_requested)
    ...
    def _open_cli_history_panel(self, conn_id: str):
        """Show the CLI history (Befehle + Sitzungen via SSHDriveMgr-cli.exe) for a host."""
        if self._panel_mode in (_PANEL_EDIT, _PANEL_ADD):
            if not self._guard_leave_form():
                return
        conn = self._mgr.get_by_id(conn_id)
    ...
    def _build_edit_form(self, conn):
        ...
        self._ef_drive.addItem("Auto (auto-select)", "AUTO")
        ...
        if is_edit and conn.drive_letter and conn.drive_letter.upper() != "AUTO":
    ...
    def _shortcut_copy(self):
        """Copy the currently selected connection to the internal clipboard."""
        target_id = self._panel_conn_id or self._selected_id
        if target_id:
            self._copy_connection(target_id)

    def _shortcut_paste(self):
        """Paste the connection from the internal clipboard as a new connection."""
        self._paste_connection()

    def _copy_connection(self, conn_id: str):
        """Store a connection id in the internal clipboard for later paste."""
        conn = self._mgr.get_by_id(conn_id)
        if not conn:
            return
        self._clipboard_conn_id = conn_id
        self._set_status(tr("copy.status.copied", name=conn.name))

    def _paste_connection(self):
        """Duplicate the clipboard connection as a new connection.

        Only appends the new card to the list instead of rebuilding the whole
        list — rebuilding every card via _refresh_list() caused visible flicker.
        """
    ...
    def _snapshot_form(self) -> dict:
        return {
            ...
            "drive": self._safe_current_data("_ef_drive", "AUTO"),
    ...
    def _ef_collect_protocol_fields(self) -> dict:
        ...
        return {
            ...
            "drive_letter": "" if is_ftp else self._safe_current_data("_ef_drive", "AUTO"),
    ...
    def _on_mount_finished(self, conn_id: str, result):
        ...
        if card:
            card.hide_loading()
        if result.success:
            # Persist the resolved drive letter when the connection was set to AUTO
            if result.drive_letter and conn and conn.drive_letter.upper() == "AUTO":
                try:
                    conn.drive_letter = result.drive_letter
                    self._mgr.update(conn)
                except Exception:
                    pass
            if card:
    ...
    @pyqtSlot(str)
    def _on_drive_change_requested(self, conn_id: str):
        """Show a popup menu to quickly pick a drive letter (or Auto) for a connection."""
        from PyQt6.QtWidgets import QMenu
        from src.drive_utils import get_available_drives, is_auto_drive
        conn = self._mgr.get_by_id(conn_id)
        if not conn or getattr(conn, "is_ftp", False):
            return
        menu = QMenu(self)
        # Auto action
        auto_act = menu.addAction("Auto (auto-select smallest free)")
        auto_act.setData("AUTO")
        auto_act.setCheckable(True)
        auto_act.setChecked(is_auto_drive(conn.drive_letter))
        menu.addSeparator()
        # Current drive (even if occupied) so it stays selectable
        used = [c.drive_letter for c in self._mgr.get_all()
                if not c.is_ftp and c.id != conn_id]
        available = get_available_drives(exclude=used)
        curr = conn.drive_letter.upper().rstrip("\\") + ":" if conn.drive_letter else ""
        if curr and curr not in available and not is_auto_drive(conn.drive_letter):
            available.insert(0, curr)
        for letter in sorted(set(available)):
            act = menu.addAction(letter)
            act.setData(letter)
            act.setCheckable(True)
            act.setChecked(letter == curr)
        chosen = menu.exec(self._cards[conn_id]._drive_badge.mapToGlobal(
            self._cards[conn_id]._drive_badge.rect().bottomLeft())) if conn_id in self._cards else menu.exec()
        if chosen is None:
            return
        drive = chosen.data()
        if not drive:
            return
        conn.drive_letter = drive
        try:
            self._mgr.update(conn)
        except Exception:
            pass
    ...
    def _on_card_context_menu(self, conn_id: str, global_pos):
        ...
        menu.addSeparator()
        act_copy = menu.addAction(tr("card.menu.copy"))
        act_paste = menu.addAction(tr("card.menu.paste"))
        act_paste.setEnabled(self._clipboard_conn_id is not None)
        menu.addSeparator()
        ...
        if act_explorer_ftp is not None and chosen == act_explorer_ftp:
            ...
            self._copy_connection(conn_id)
        elif act_paste is not None and chosen == act_paste:
            self._paste_connection()
