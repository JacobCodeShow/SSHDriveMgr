...
Update strategy (since 1.5.4):
    ...
    Instead the installer is stored in %APPDATA%\\SSHDriveMgr\\updates together
...
def updates_dir() -> Path:
    ...
    return Path(appdata) / "SSHDriveMgr" / "updates"
...
class UpdaterManager(QObject):
    ...
    def check_for_updates_async(self):
        ...
        def _worker():
            try:
                req = urllib.request.Request(GITHUB_API_URL, headers={"User-Agent": "SSHDriveMgr-Updater"})
    ...
    def download_update_async(self, download_url: str):
        ...
        def _worker():
            try:
                ...
                file_name = self._asset_name or "SSHDriveMgr-Setup.exe"
                self.update_file_path = str(target_dir / file_name)

                req = urllib.request.Request(download_url, headers={"User-Agent": "SSHDriveMgr-Updater"})
    ...
    def _verify_checksum(self):
        ...
        try:
            cs_req = urllib.request.Request(checksum_url, headers={"User-Agent": "SSHDriveMgr-Updater"})
