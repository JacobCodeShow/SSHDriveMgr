"""
cli_main.py - Console entry point for SSH 磁盘管理器 CLI access.

Built as a separate console-subsystem EXE (SSHDriveMgr-cli.exe) so that
...
Usage:
    SSHDriveMgr-cli.exe --connect-cli <access_key>
    SSHDriveMgr-cli.exe --connect-cli -          # liest Key von stdin (empfohlen)
"""
...
def main() -> int:
    ...
    if not key:
        print("Usage: SSHDriveMgr-cli.exe --connect-cli <access_key> [--exec \"command\"]")
        print("       SSHDriveMgr-cli.exe --connect-cli - [--exec \"command\"]  # Key via stdin")
        return 2
